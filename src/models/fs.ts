...
export type AspectFnThis<A extends {} = {}> = ({ ctor?: Function } & Partial<A>) | void;
export type AspectFn<I extends {}[], A extends {}> = (this: AspectFnThis<A>, ...init: I) => Promise<A & { ctor?: AspectFn<I, A> }>;
export const makeAspectCtor = <I extends {}[], A extends {}>(aspectFn: AspectFn<I, A>) => {
    async function aspectCtor(this: AspectFnThis<A>, ...init: I) {
        const aspect = await aspectFn.call(this ?? { ctor: aspectCtor }, ...init);
        return ({
            [aspect.ctor?.name ?? aspectFn.name]: aspect
        });
    }
...
export const Hash = async ({ file, hash }: { file: File, hash?: { sha256: string, _ts: number } }) =>
